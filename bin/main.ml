...
let parse_client_username () =
  if Array.length Sys.argv <> 3 then fatal_error "Please enter a username.";
  if String.starts_with ~prefix:"\"" Sys.argv.(1) then
    String.sub Sys.argv.(2) 1 (String.length Sys.argv.(2) - 2)
  else Sys.argv.(2)
...
let rec read_print_n_lines n output_channel =
  if n = 0 then Lwt.return ()
  else
    let%lwt line = Lwt_io.read_line output_channel in
    let%lwt () = Lwt_io.printlf "%s" line in
    read_print_n_lines (n - 1) output_channel
...
let run_client () =
  let client () =
    ...
    (* initialize game *)
    let rec send () =
      let%lwt () = Lwt_io.print "Send a message: " in
      let%lwt message_opt = Lwt_io.read_line_opt Lwt_io.stdin in
      match message_opt with
      | None -> fatal_error "Input closed. Exiting client."
      | Some message ->
          Lwt.catch
            (fun () ->
              let%lwt () = Lwt_io.write_line server_out message in
              let%lwt () = Lwt_io.flush server_out in
              send ())
            (function
              | Lwt_io.Channel_closed _ | Unix.Unix_error (_, _, _) ->
                  fatal_error "Server disconnected."
              | exn -> fatal_error ("Error: " ^ Printexc.to_string exn))
    in
